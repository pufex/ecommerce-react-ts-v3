...
const PayForm = () => {
    ...
    const [name, handleNameChange, setNameError] = useInput({})
    const [surname, handleSurnameChange, setSurnameError] = useInput({})
    const [address, handleAddressChange, setAddressError] = useInput({})
    const [postCode, handleCodeChange, setCodeError] = useInput({})
    const [city, handleCityChange, setCityError] = useInput({})
    const [email, handleEmailChange, setEmailError] = useInput({})
    const [phone, handlePhoneChange, setPhoneError] = useInput({})
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const [_phone, phoneError] = valid.validatePhone(phone.value)
        if(phoneError){
            shouldReturn = true;
            if(navigateTo == 0)
                navigateTo = 1;
            setPhoneError(true, "Invalid Phone")
        }
